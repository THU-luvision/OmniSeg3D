...
std::vector<torch::Tensor> composite_sam_train_fw(
    const torch::Tensor sigmas,
    const torch::Tensor rgbs,
    ...
    return composite_sam_train_fw_cu(
                sigmas, rgbs, sams, deltas, ts,
                rays_a, opacity_threshold, sam_feature_dim);
}


std::vector<torch::Tensor> composite_sam_train_bw(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_drgb,
    ...
    return composite_sam_train_bw_cu(
                dL_dopacity, dL_ddepth, dL_drgb, dL_dsam, dL_dws,
                sigmas, rgbs, sams, ws, deltas, ts, rays_a,
                opacity, depth, rgb, sam, opacity_threshold, sam_feature_dim);
}


void composite_sam_test_fw(
    const torch::Tensor sigmas,
    const torch::Tensor rgbs,
    const torch::Tensor sams,
    const torch::Tensor deltas,
    ...
    composite_sam_test_fw_cu(
        sigmas, rgbs, sams, deltas, ts, hits_t, alive_indices,
        T_threshold, sam_feature_dim, N_eff_samples,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("composite_sam_train_fw", &composite_sam_train_fw);
    m.def("composite_sam_train_bw", &composite_sam_train_bw);
    m.def("composite_sam_test_fw", &composite_sam_test_fw);
