...
__global__ void composite_sam_train_fw_kernel(
    ...
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> sams,
    ...
    const scalar_t sam_feature_dim,
    ...
    torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> sam,
    ...
    while (samples < N_samples) {
        ...
        for (int i=0; i<sam_feature_dim; i++) {
            sam[ray_idx][i] += w*sams[s][i];
        }
...
std::vector<torch::Tensor> composite_sam_train_fw_cu(
    ...
    auto sam = torch::zeros({N_rays, sam_feature_dim}, sigmas.options());
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_sam_train_fw_cu", 
    ([&] {
        composite_sam_train_fw_kernel<scalar_t><<<blocks, threads>>>(
            ...
            sams.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            ...
            sam.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            ws.packed_accessor<scalar_t, 1, torch::RestrictPtrTraits, size_t>()
        );
    }));

    return {total_samples, opacity, depth, rgb, sam, ws};
}


template <typename scalar_t>
__global__ void composite_sam_train_bw_kernel(
    ...
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> dL_dsam,
    ...
    const torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> sam,
    const scalar_t T_threshold,
    const scalar_t sam_feature_dim,
    ...
    torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> dL_dsams,
    torch::PackedTensorAccessor<scalar_t, 2, torch::RestrictPtrTraits, size_t> sam_acc
){
    const int n = blockIdx.x * blockDim.x + threadIdx.x;
    if (n >= opacity.size(0)) return;
    ...
    while (samples < N_samples) {
        ...
        for (int i=0; i<sam_feature_dim; i++) {
            dL_dsams[s][i] = dL_dsam[ray_idx][i]*w;
        }

        sam_acc_grad = 0.0f;
        for (int i=0; i<sam_feature_dim; i++) {
            sam_acc_grad += dL_dsam[ray_idx][i]*(sams[s][i]*T-(sam[ray_idx][i]-sam_acc[s][i]));
        }
        ...
        dL_dsigmas[s] = deltas[s] * (
            ...
            sam_acc_grad + // gradients from sam
...
std::vector<torch::Tensor> composite_sam_train_bw_cu(
    ...
    auto dL_dsams = torch::zeros({N, sam_feature_dim}, sigmas.options());
    auto sam_acc = torch::zeros({N, sam_feature_dim}, sigmas.options());

    auto dL_dws_times_ws = dL_dws * ws; // auxiliary input

    const int threads = 256, blocks = (N_rays+threads-1)/threads;

    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_sam_train_bw_cu", 
    ([&] {
        composite_sam_train_bw_kernel<scalar_t><<<blocks, threads>>>(
            ...
            dL_dsam.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            ...
            dL_dsams.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>(),
            sam_acc.packed_accessor<scalar_t, 2, torch::RestrictPtrTraits, size_t>()
        );
    }));

    return {dL_dsigmas, dL_drgbs, dL_dsams};
}


template <typename scalar_t>
__global__ void composite_sam_test_fw_kernel(
    ...
    const torch::PackedTensorAccessor<scalar_t, 3, torch::RestrictPtrTraits, size_t> sams,
...
void composite_sam_test_fw_cu(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(sigmas.type(), "composite_sam_test_fw_cu", 
    ([&] {
        composite_sam_test_fw_kernel<scalar_t><<<blocks, threads>>>(
            ...
            sams.packed_accessor<scalar_t, 3, torch::RestrictPtrTraits, size_t>(),
