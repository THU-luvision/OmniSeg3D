...
root_dir=/data/haiyang/projects/Datasets/360/counter
exp_name=0113_counter
dataset_name=colmap
bg_color=2
scale=3
downsample=0.25
...
if test $opt == "train_rgb"; then
    ...
    python train.py \
        ...
        --semantic_dim 16 \
        --semantic_only \
        --weight_path results/${dataset_name}/${exp_name}/rgb/ckpts/epoch=$(expr ${num_epoch_rgb} - 1).ckpt
    ...
    python show_gui_multi_4box.py \
        ...
        --semantic_flag \
        --semantic_dim 16 \
        --ckpt_path results/${dataset_name}/${exp_name}/sem/ckpts/epoch=$(expr ${num_epoch_sem} - 1)_slim.ckpt
