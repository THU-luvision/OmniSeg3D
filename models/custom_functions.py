...
class VolumeRenderer_sam(torch.autograd.Function):
    """
    Volume rendering with different number of samples per ray
    Used in training only
    ...
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, sams, deltas, ts, rays_a, T_threshold, sam_feature_dim):
        total_samples, opacity, depth, rgb, sam, ws = \
            vren.composite_sam_train_fw(sigmas, rgbs, sams, deltas, ts,
                                    rays_a, T_threshold, sam_feature_dim)
        ctx.save_for_backward(sigmas, rgbs, sams, deltas, ts, rays_a,
                              opacity, depth, rgb, sam, ws)
        ctx.T_threshold = T_threshold
        ctx.sam_feature_dim = sam_feature_dim
        return total_samples.sum(), opacity, depth, rgb, sam, ws

    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dsam, dL_dws):
        sigmas, rgbs, sams, deltas, ts, rays_a, \
        opacity, depth, rgb, sam, ws = ctx.saved_tensors
        dL_dsigmas, dL_drgbs, dL_dsams = \
            vren.composite_sam_train_bw(dL_dopacity, dL_ddepth, dL_drgb, dL_dsam, dL_dws,
                                    sigmas, rgbs, sams, ws, deltas, ts,
                                    rays_a,
                                    opacity, depth, rgb, sam,
                                    ctx.T_threshold, ctx.sam_feature_dim)
        return dL_dsigmas, dL_drgbs, dL_dsams, None, None, None, None, None


class VolumeRenderer_sem(torch.autograd.Function):
    """
    Volume rendering with different number of samples per ray
    Used in training only
    ...
    def forward(ctx, sigmas, rgbs, semantics, deltas, ts, rays_a, T_threshold):
        total_samples, opacity, depth, rgb, semantic, ws = \
            vren.composite_sem_train_fw(sigmas, rgbs, semantics, deltas, ts,
                                    rays_a, T_threshold)
        ctx.save_for_backward(sigmas, rgbs, semantics, deltas, ts, rays_a,
                              opacity, depth, rgb, semantic, ws)
        ctx.T_threshold = T_threshold
        return total_samples.sum(), opacity, depth, rgb, semantic, ws

    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dsemantic, dL_dws):
        sigmas, rgbs, semantics, deltas, ts, rays_a, \
        opacity, depth, rgb, semantic, ws = ctx.saved_tensors
        dL_dsigmas, dL_drgbs, dL_dsemantics= \
            vren.composite_sem_train_bw(dL_dopacity, dL_ddepth, dL_drgb, dL_dsemantic, dL_dws,
