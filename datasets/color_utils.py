...
def read_depth(dep_path, dep_wh, depth_scale=1/6553.5):
    dep = imageio.imread(dep_path).astype(np.float32) * depth_scale

    dep = cv2.resize(dep, dep_wh)
    dep = dep[..., None]
    dep = rearrange(dep, 'h w c -> (h w) c')
